...
public class BaseCampManager : MonoBehaviour
{
    ...
    {
        if (commandCenter != null && commandCenter.TryUseBossTicket())
        {
    ...
    {
        int currentLevel = commandCenter != null ? commandCenter.Level : 1;
    ...
    {
        DailyMissionManager manager = DailyMissionManager.Instance ?? FindFirstObjectByType<DailyMissionManager>();
        if (manager == null)
        {
            manager = gameObject.AddComponent<DailyMissionManager>();
        }
    ...
    private JinyouSaveData CaptureUnifiedSaveData()
    {
        ...
        DailyMissionManager dailyMissionManager = EnsureDailyMissionManager();
        data.dailyMissions = dailyMissionManager != null ? dailyMissionManager.CaptureState() : new JinyouDailyMissionSaveData();
        return data;
    }
    ...
    private void ApplyOfflineRewards(JinyouSaveData data)
    {
        ...
        float maxOfflineSeconds = commandCenter != null
            ? Mathf.Max(0f, commandCenter.OfflineRewardLimitHours) * 3600f
            : 0f;
        ...
        lastOfflineReward = new JinyouOfflineRewardSaveData
        {
            ...
            bossTicketsAdded = commandCenter != null ? commandCenter.ProduceBossTicketsOffline(appliedSeconds) : 0
        };
    ...
    private void SubscribeUnifiedSaveEvents()
    {
        ...
        DailyMissionManager dailyMissionManager = EnsureDailyMissionManager();
        dailyMissionManager?.OnDailyMissionsChanged.AddListener(HandleUnifiedSaveEvent);
        dailyMissionManager?.OnDailyMissionCompleted.AddListener(HandleUnifiedSaveEvent);
        dailyMissionManager?.OnDailyMissionRewardClaimed.AddListener(HandleUnifiedSaveEvent);
    }
    ...
    private void UnsubscribeUnifiedSaveEvents()
    {
        ...
        DailyMissionManager dailyMissionManager = DailyMissionManager.Instance ?? FindFirstObjectByType<DailyMissionManager>();
